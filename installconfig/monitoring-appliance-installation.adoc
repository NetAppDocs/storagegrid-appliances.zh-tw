...
. 繼續監控安裝進度，直到*安裝StorageGRID*階段暫停並且嵌入式控制台上出現一條訊息，提示您使用網格管理器在管理節點上批准此節點；然後，請轉到下一步。
. https://docs.netapp.com/us-en/storagegrid/admin/signing-in-to-grid-manager.html["前往 Grid Manager"^]主管理節點，批准待處理的儲存節點，並完成StorageGRID安裝流程。
+
...
+
** 對於除主管理節點之外的所有裝置節點，「安裝StorageGRID」階段將暫停，且嵌入式控制台上會出現一則訊息，提示您使用 https://docs.netapp.com/us-en/storagegrid/admin/signing-in-to-grid-manager.html["網格管理程式"^]；然後，轉到下一步。
