...
此外，在啟用節點加密後啟用 FIPS 模式時， NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 模組用於靜態資料加密。參考 https://review.docs.netapp.com/us-en/storagegrid_storagegrid-120-dev/admin/manage-tls-ssh-policy.html#select-a-security-policy["選擇安全策略"^]了解更多。
...
CAUTION: 當安裝啟用節點加密的裝置時，會指派一個臨時金鑰。在裝置連接到金鑰管理系統 (KMS) 並設定 KMS 安全金鑰之前，裝置上的資料不受保護。請參閱 https://docs.netapp.com/us-en/storagegrid/admin/kms-overview-of-kms-and-appliance-configuration.html["KMS 應用裝置組態總覽"^]了解更多。
...
當您為具有 FIPS 磁碟機的StorageGRID裝置啟用磁碟機加密時，FIPS 磁碟機提供的 FIPS 加密將用於靜態資料的加密。

您可以在載入 Grid Manager 之前在初始裝置安裝期間啟用磁碟機加密。您也可以將裝置置於維護模式來啟用磁碟機加密或變更密碼。
