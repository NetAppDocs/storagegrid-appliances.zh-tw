...
NOTE: 對連接埠重新映射的支援已棄用，並將在未來的版本中刪除。若要刪除重新映射的端口，請參閱 https://docs.netapp.com/us-en/storagegrid/maintain/removing-port-remaps.html["刪除StorageGRID設備上的連接埠重新映射"^]。
...
+
此處顯示的名稱將是設備節點的系統名稱。系統名稱是內部StorageGRID操作所必需的，並且不能更改。若要變更設備節點的顯示名稱，請參閱 https://docs.netapp.com/us-en/storagegrid/maintain/rename-grid-site-node.html#how-to-rename-grid-sites-or-nodes["如何重新命名網格、網站或節點"^]。
...
.接下來呢？
了解 link:https://docs.netapp.com/us-en/storagegrid/swnodes/navigating-to-grid-manager.html["配置網格並完成安裝"^] 和 link:monitoring-appliance-installation.html["監控設備安裝"]。
