...
.開始之前
* 您有 link:verify-component-to-replace-sg6000cn.html["已驗證需要更換 CMOS 電池的設備"]。
* 您可以 link:locating-controller-in-data-center.html["實體放置 SG6000-CN 控制器"] 在資料中心更換 CMOS 電池。
+

NOTE: link:power-sg6000-cn-controller-off-on.html["受控制的產品關機"]從機架中取出產品之前、必須先使用。

* 您已拔下所有纜線和 link:reinstalling-sg6000-cn-controller-cover.html["已取下產品護蓋"]。
...
CAUTION: 如果您曾經使用過僅建立一個物件複本的 ILM 規則、則必須在排程維護期間更換電池、因為在此程序期間可能會暫時失去這些物件的存取權。請參閱的相關資訊 https://docs.netapp.com/us-en/storagegrid/ilm/why-you-should-not-use-single-copy-replication.html["為何不應使用單一複本複寫"^]。
