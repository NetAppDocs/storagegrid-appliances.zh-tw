...
.. link:reinstalling-sgf6112-into-cabinet-or-rack.html["從機櫃或機架移除故障控制器"] 。
.. 請注意故障控制器中可更換組件（兩個電源、三個 NIC 和兩個 SSD）的位置。
+
兩個磁碟機位於機箱中的下列位置（圖示為卸下擋板的機箱正面）：

+
image::../media/sg6100cn_ssds_locations.png[磁碟機位置]
...
CAUTION: 如果要保留磁碟機上的數據，請將 SSD 磁碟機插入故障裝置中佔用的相同磁碟機插槽。如果不這樣做，設備安裝程式將顯示警告，您必須將驅動器放入正確的插槽並重新啟動控制器，然後控制器才能重新加入電網。

.. link:reinstalling-sgf6112-into-cabinet-or-rack.html["將替換控制器安裝到機櫃或機架中"] 。
.. 更換纜線和任何光纖收發器。
.. 開啟控制器電源並監控控制器 LED 。
