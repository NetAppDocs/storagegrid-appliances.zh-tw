...
.關於這項工作
若要防止服務中斷，請在開始更換網路介面卡 (NIC) 之前確認所有其他儲存節點都已連線至電網，或在可以接受服務中斷時間的計畫維護時段內更換 NIC。查看有關 https://docs.netapp.com/us-en/storagegrid/monitor/monitoring-system-health.html#monitor-node-connection-states["繫泊節點連線狀態"^]。
